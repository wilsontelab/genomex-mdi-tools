...
our %baseMatches =  ( # score with full matchScore in SW
    AA => 'A',
    CC => 'C',
    GG => 'G',
    TT => 'T',    
);
our %baseMismatches = ( # score with full mismatchPenalty in SW
    AG => 'R',
    GA => 'R',
...
our %ryswkmMatches = ( # score with half matchScore in SW
    RR => 'R',
    RA => 'R',
    RG => 'R',
    AR => 'R',
...
our %nMatches = ( # score with neutral=0 in SW (neither penalize nor promote)
...
our %mOperations = (%baseMatches, %baseMismatches, %ryswkmMatches);

# combinations for doing ungapped comparisons via sub noIndelMatch
our %ACGTNMatches   = (%baseMatches, %nMatches);
our %noIndelMatches = (%baseMatches, %baseMismatches, %nMatches);
